...
PTR(Val) IfExpr::interp(PTR(Env) env) {
    PTR(BoolVal) if_value = CAST(BoolVal)(if_part->interp(env));
    
    if (if_value == NULL)
        throw std::runtime_error("the condition is illegal");
    else if (if_value->equals(NEW(BoolVal)(true))) {
...
PTR(Expr) IfExpr::optimize() {
    ...
    if (if_part->haveVariable()) {
        return NEW(IfExpr)(if_part->optimize(), then_part->optimize(), else_part->optimize());
    }
    
    PTR(BoolExpr) if_part_optimized = CAST(BoolExpr)(if_part->optimize());
    
    if (if_part_optimized == NULL)
        throw std::runtime_error("the condition is illegal");
    else if (if_part_optimized->equals(NEW(BoolExpr)(true)))
